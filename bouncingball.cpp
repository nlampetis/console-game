...
const CHAR_INFO &Bouncy::getBouncyCharInfo() const { return chInfo; }

void Bouncy::draw(Console &cl) {
  COORD temp = {static_cast<SHORT>(floor(pos.x)),
                static_cast<SHORT>(floor(pos.y))};
  cl.updateBuffer(temp, chInfo.Char.AsciiChar, chInfo.Attributes);
}
